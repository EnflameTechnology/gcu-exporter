...
function checkCommand() {
    if [ "$CLI_NAME" == "docker" ]; then
        ...
        if ! command -v podman > /dev/null 2>&1; then
            echo "podman not found, please install podman first!"; exit 1
        fi
    elif [ "$CLI_NAME" == "nerdctl" ]; then
        if ! command -v nerdctl > /dev/null 2>&1; then
            echo "nerdctl not found, please install nerdctl first!"; exit 1
        fi
    elif [ "$CLI_NAME" == "ctr" ]; then
        if ! command -v ctr > /dev/null 2>&1; then
            echo "ctr not found, please install ctr first!"; exit 1
        fi
        if ! command -v docker > /dev/null 2>&1; then
            echo "docker not found, please install docker first!"; exit 1
        fi
